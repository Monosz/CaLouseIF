...
public class DeclineItemView extends BorderPane{
	private Stage stage;
	private User user;
	private Item item;
	...
	private void init() {
		...
		submitReasonButton = new Button("Decline Item");
	...
	private void setEvents() {
		...
		submitReasonButton.setOnMouseClicked(e -> {
			...
			int res = ItemController.declineItem(item.getId());
			if (res == 1) {
				new HomeView(stage);				
			} else {
	...
	public DeclineItemView(Stage stage, int id) {
		this.stage = stage;
